...
from MPRNet_improve import MPRNet
import losses
...
train_dir = os.path.join(opt.TRAINING.TRAIN_DIR, args.train_dataset)
...
val_loader = DataLoader(dataset=val_dataset, batch_size=2, shuffle=False, num_workers=2, drop_last=False, pin_memory=True)
...
with open(history_path, "w") as f:
    f.write("epoch, train_loss, val_psnr, learning_rate\n")
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    if epoch%opt.TRAINING.VAL_AFTER_EVERY == 0:
        val_psnr_to_log = psnr_val_rgb
    else:
        val_psnr_to_log = float('nan')

    with open(history_path, "a") as f:
        f.write(f"{epoch}, {epoch_loss}, {val_psnr_to_log}, {current_lr}\n")
