...
from ops.irnn_layer import IRNN
...
####################################################################
## Hybrid SPA-Net Supervised Attention Module (CUDA Accelerated)
## 結合 MPRNet 的圖片監督與 SPA-Net 的空間遞歸注意力
class Hybrid_SPA_SAM(nn.Module):
    def __init__(self, n_feat, kernel_size=3, bias=True):
        super(Hybrid_SPA_SAM, self).__init__()
        
        # --- 分支 A: 產出圖片 (維持 MPRNet 的監督機制) ---
        # 負責從特徵預測殘差圖，加上去變成修復影像
        self.conv_img = conv(n_feat, 3, kernel_size, bias=bias)
        
        # --- 分支 B: IRNN 空間注意力 (SPA-Net 核心) ---
        # 1. 特徵前處理
        self.conv_in = nn.Conv2d(n_feat, n_feat, 3, padding=1, bias=bias)
        
        # 2. 呼叫我們用 C++/CUDA 編譯好的 IRNN
        self.spatial_irnn = IRNN(n_feat)
        
        # 3. 產生 Attention Mask (0~1)
        self.conv_mask = nn.Conv2d(n_feat, n_feat, 1)
        self.sigmoid = nn.Sigmoid()
        
        # 4. 輸出層
        self.conv_out = nn.Conv2d(n_feat, n_feat, 3, padding=1, bias=bias)

    def forward(self, x, x_img):
        # x: 輸入特徵
        # x_img: 上一階段的影像 (Input Image or Stage Result)

        # --- 任務 1: 計算 Loss 用的影像 ---
        # 這裡邏輯與原版 SAM 保持一致: img = conv(x) + x_img
        img_residual = self.conv_img(x)
        img = img_residual + x_img
        
        # --- 任務 2: 空間注意力機制 (取代原版 SAM 的 attention) ---
        # 先做卷積
        feat = self.conv_in(x)
        
        # 進入 CUDA 加速的 IRNN (這裡會飛快)
        feat_irnn = self.spatial_irnn(feat)
        
        # 產生 Mask
        mask = self.sigmoid(self.conv_mask(feat_irnn))
        
        # 應用 Mask (Residual Connection)
        # 原版 SAM 是 x * mask + x，我們保持這個邏輯
        x_refined = x * mask + x
        
        # 最後整理特徵
        x_out = self.conv_out(x_refined)
        
        return x_out, img

##########################################################################
class MPRNet_improve(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=40, scale_unetfeats=20, scale_orsnetfeats=16, num_cab=8, kernel_size=3, reduction=4, bias=False):
        super(MPRNet_improve, self).__init__()
        ...
        # 注意: 我們這裡 kernel_size 改用 3 (SPA-Net 偏好 3x3)，原版 SAM 是 1x1
        self.sam12 = Hybrid_SPA_SAM(n_feat, kernel_size=3, bias=bias)
        self.sam23 = SAM(n_feat, kernel_size=1, bias=bias)
